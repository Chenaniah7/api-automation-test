...
public class RestDeleteJob extends AbstractRestJob {
    private static final Logger LOGGER = LoggerFactory.getLogger(RestDeleteJob.class.getName());

    public void perform(Entity entity) {
        if (entity.getEndPoint() == null) {
            final String msg = "DELETE api endpoint can't be null";
            RestDeleteJob.LOGGER.error(msg, new IllegalArgumentException());
            return;
        }
        ...
        final ValidatableResponse response = httpRequest.when().delete(entity.getEndPoint()).then().log().all();
        ...
        entity.setResponseHeaders(response.extract().response().headers().asList().stream().collect(Collectors.toMap(Header::getName, Header::getValue)));
    }
